...
const DEFAULT_PATTERNS: StructuralPattern[] = [
  {
...
export class PatternLibrary {
  private patterns: Map<string, StructuralPattern>;
   ...
   * Get pattern by ID
   */
  get(id: string): StructuralPattern | undefined {
   ...
   */
  getByDomain(domain: string): StructuralPattern[] {
    return Array.from(this.patterns.values()).filter(
      p => p.source_domain.toLowerCase().includes(domain.toLowerCase())
    );
  }
   ...
   */
  search(keyword: string): StructuralPattern[] {
    const lower = keyword.toLowerCase();
    return Array.from(this.patterns.values()).filter(p =>
      p.source_domain.toLowerCase().includes(lower) ||
   ...
   * Add a new pattern
   */
  async add(pattern: Omit<StructuralPattern, "id" | "created" | "usage_count">): Promise<StructuralPattern> {
    const id = pattern.source_domain.toLowerCase().replace(/\s+/g, "_") + "_" + Date.now();
    const newPattern: StructuralPattern = {
      ...pattern,
      id,
      created: Date.now(),
      usage_count: 0
   ...
   */
  async strengthen(id: string): Promise<void> {
  ...
  getStats(): {
    total: number;
    most_used: Array<{ id: string; source_domain: string; usage_count: number }>;
    domains: string[];
  } {
...
// Singleton instance
let patternLibraryInstance: PatternLibrary | null = null;

export async function getPatternLibrary(): Promise<PatternLibrary> {
