...
export class EpisodeBank {
  ...
  async init(): Promise<void> {
    try {
      ...
      const parsed: EpisodeData = JSON.parse(data);
      this.episodes.clear();
   ...
   * Store a new episode
   */
  async store(episode: Omit<Episode, "id" | "created" | "access_count">): Promise<Episode> {
   ...
   */
  retrieve(query: string, maxResults: number = 3, minSimilarity: number = 0.2): Array<{
    episode: Episode;
   ...
   */
  getByDomain(domain: string): Episode[] {
    return Array.from(this.episodes.values()).filter(
      ep => ep.metadata.domain?.toLowerCase() === domain.toLowerCase()
    );
  }
   ...
   */
  getByOutcome(outcome: "success" | "partial" | "failure"): Episode[] {
  ...
  getStats(): {
    total: number;
    byOutcome: Record<"success" | "partial" | "failure", number>;
    byDomain: Record<string, number>;
    avgAccessCount: number;
  } {
    const episodes = Array.from(this.episodes.values());
    const byOutcome: Record<"success" | "partial" | "failure", number> = {
      success: 0,
      partial: 0,
      failure: 0
    };
...
// Singleton instance
let episodeBankInstance: EpisodeBank | null = null;

export async function getEpisodeBank(): Promise<EpisodeBank> {
