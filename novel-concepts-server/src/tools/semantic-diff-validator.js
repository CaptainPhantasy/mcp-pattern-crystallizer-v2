...
function extractFunctionSignatures(code, language = "javascript") {
    const signatures = [];
    const patterns = [
...
function compareSignatures(oldSig, newSig) {
    const changes = [];
    let breaking = false;
    // Name change
...
function analyzeSemanticChanges(files) {
    const changes = [];
    for (const file of files) {
        for (const hunk of file.hunks) {
            ...
            for (const oldSig of oldSignatures) {
                ...
                const matchingNew = newSignatures.find(ns => ns.name === oldSig.name ||
            ...
            for (const newSig of newSignatures) {
                const hasOldMatch = oldSignatures.some(os => os.name === newSig.name ||
...
function estimateAffectedCallers(changes, validation_depth) {
    const callers = [];
...
function generateTests(changes) {
    const tests = [];
    for (const change of changes) {
...
function calculateRiskScore(changes, callers, validation_depth) {
    let risk = 0;
...
export const semanticDiffValidatorDefinition = {
...
export async function handleSemanticDiffValidator(args) {
    try {
