...
export type ExecutionTraceSynthesizerInput = z.infer<typeof ExecutionTraceSynthesizerInputSchema>;
...
interface PotentialIssue {
  type: "null_dereference" | "infinite_loop" | "unreachable" | "type_mismatch" | "missing_return";
  location: string;
  scenario: string;
  confidence: number;
  description: string;
...
const LANGUAGE_PATTERNS: Record<string, {
  ...
  variableAssignment: RegExp;
  propertyAccess: RegExp;
}> = {
  javascript: {
...
function extractStructure(code: string, language: string): {
  functions: Array<{ name: string; params: string[]; startLine: number }>;
  conditionals: Array<{ condition: string; line: number }>;
  loops: Array<{ condition: string; line: number }>;
  returns: Array<{ value: string; line: number }>;
  assignments: Array<{ variable: string; value: string; line: number }>;
  propertyAccesses: Array<{ object: string; property: string; line: number }>;
} {
  const patterns = LANGUAGE_PATTERNS[language] || LANGUAGE_PATTERNS.javascript;
  const lines = code.split("\n");

  const functions: Array<{ name: string; params: string[]; startLine: number }> = [];
  const conditionals: Array<{ condition: string; line: number }> = [];
  const loops: Array<{ condition: string; line: number }> = [];
  const returns: Array<{ value: string; line: number }> = [];
  const assignments: Array<{ variable: string; value: string; line: number }> = [];
  const propertyAccesses: Array<{ object: string; property: string; line: number }> = [];
...
function evaluateCondition(condition: string, state: Record<string, unknown>): boolean {
...
function evaluateExpression(expr: string, state: Record<string, unknown>): unknown {
  ...
  if (propertyMatch) {
    ...
    if (obj && typeof obj === "object") {
      return (obj as Record<string, unknown>)[propertyMatch[2]];
    }
  }
...
export const executionTraceSynthesizerDefinition: Tool = {
...
export async function handleExecutionTraceSynthesizer(args: unknown) {
  try {
