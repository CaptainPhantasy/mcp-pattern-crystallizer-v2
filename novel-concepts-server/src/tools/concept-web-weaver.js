...
// JSON Schema for MCP
export const conceptWebWeaverDefinition = {
 ...
 */
export async function handleConceptWebWeaver(args) {
    try {
        ...
        switch (input.action) {
            case "register": {
                ...
                const conceptId = graph.register(input.concept, input.relationships || [], input.metadata);
                return {
                    content: [{
                            type: "text",
                            text: JSON.stringify({
