...
function calculateStructuralSimilarity(problemStructure, pattern) {
    let score = 0;
    let maxScore = 0;
...
function generateMapping(pattern, problemDescription) {
    const mapping = [];
    // Common mappings
    const mappings = [
        {
    ...
    if (mapping.length < 3) {
        mapping.push({ source_feature: "central coordination point", target_feature: "orchestrator/coordinator" }, { source_feature: "worker/unit of work", target_feature: "task/job" });
    }
    return mapping;
}
// Generate transferable insights
function generateInsights(pattern, problemDescription) {
    const insights = [];
...
function generateApproach(pattern, insights, problemDescription) {
    const lower = problemDescription.toLowerCase();
...
export const analogySynthesizerDefinition = {
...
export async function handleAnalogySynthesizer(args) {
    try {
        ...
        if (input.source_domains && input.source_domains.length > 0) {
            patterns = patterns.filter(p => input.source_domains.some(d => p.source_domain.toLowerCase().includes(d.toLowerCase())));
        }
        ...
        const suggestedApproach = bestAnalogy
            ? generateApproach({ source_domain: bestAnalogy.source_domain, abstract_structure: bestAnalogy.structural_match }, bestAnalogy.transferable_insights, input.problem_description)
